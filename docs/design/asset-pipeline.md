...
| Card-frame plates | the DOM card frame (`card/dom/`) | seven nine-sliceable **alpha light maps** — edge, art seam, header field, information strip, status channel, P/T plate, identity material | ~46 KB total; on every card, so first-match and never deferred. Carry no body colour, so one set serves every theme and all eight identities. Generated by `clients/web/scripts/generateFramePlates.js` (class 1). See `card-representation.md` §3.12 |
