...
| Card-frame plates | the DOM card frame (`card/dom/`) | seven nine-sliceable **alpha light maps** — edge, art seam, header field, information strip, status channel, P/T plate, identity material | ~62 KB total (PNG, so the committed bytes are reproducible — see §3.12); on every card, so first-match and never deferred. Carry no body colour, so one set serves every theme and all eight identities. Generated by `clients/web/scripts/generateFramePlates.js` (class 1). See `card-representation.md` §3.12 |
