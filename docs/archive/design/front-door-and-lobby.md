...
- **Match → Lobby (postgame).** The verdict is part of the terminal `GameView`
  ...
  **last-match ribbon**.
  ...
  > (`crates/sage-server/src/room/connection.rs:18-24`), so the lobby never sees
...
(`crates/sage-server/src/lobby/registry.rs:44-58`), and there is no command that
...
20. No `LobbyCommand`, `LobbyView` field, or protocol type changes in the PR
    (`sage-protocol`, `docs/protocol.md`, and the TS mirror are untouched).
...
3. **A real rematch.** Returning a finished room to `gathering`, or a
   ...
   protocol change (ADR 0012 amendment + `sage-protocol` + `docs/protocol.md` +
