...
- **Direct activation shortcuts the round trip where intent is unambiguous**
  (ADR 0025), with the same activation vocabulary in every input mode: a
  combat-declaration candidate enters the declaration pre-toggled, and activating
  an already-selected entity again fires its sole action. Mana abilities
  deliberately use that select-then-activate path too (#463), so a stray first
  press cannot irreversibly tap a land; the dock is the labeled single-press
  confirmation after selection and remains the only home for ambiguous actions.
