...
The engine ships one embedded file, `crates/sage-engine/data/cards.json`: a flat
...
`CardId` (`crates/sage-engine/src/card.rs`, ADR 0006, ADR 0007). That shape has
...
- **Zero I/O in the engine.** `sage-engine` has no tokio, no sockets, no
...
(`crates/sage-engine/src/id.rs`) *is* the `OracleId`: it already keys
...
- **`crates/sage-engine/data/oracle.json`** — an array of oracle-card records
  (the current `CardData` fields keyed by `OracleId`). One entry per distinct
  card, regardless of how many sets print it.
- **`crates/sage-engine/data/sets/<SET>.json`** — one file per set, named by set
...
- **`GameSetup` — engine data (a pure value type in `sage-engine`).** It holds
...
- **Format config — server configuration (`sage-server`).** The server owns a
