...
- `personalized_view` requires a seat — it takes a `viewer: PlayerId` and indexes
  `state.players.get(viewer.0)` to build `my_hand` (`crates/sage-server/src/view.rs`).
...
- `GameView` is defined *around its receiver*: `you` (the receiver's `p{N}` id),
  ...
  `valid_actions` all describe one seated player (`sage-protocol`
...
- Join is a **lobby command** (`JoinRoom` gains an observer variant, or a sibling
  ...
  `sage-protocol` + the TS mirror together, per the contract-discipline hard rule).
...
- **Cost: a parallel type and a second client entry path.** `sage-protocol` gains
...
- **Extends ADR 0012 rather than reopening it.** The lobby's deferred "spectators"
  ...
  shapes (contract change: `docs/protocol.md` + `sage-protocol` + TS mirror in one
