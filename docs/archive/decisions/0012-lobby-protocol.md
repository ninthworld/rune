...
empty decks (`crates/sage-server/src/lobby.rs`, `docs/roadmap.md` baseline). There
...
"`docs/protocol.md` documents the lobby contract; `sage-protocol` round-trips it;
...
is constructed. The rules below are what `sage-protocol`, `sage-server`, and the
...
the wire format stays serde JSON of `sage-protocol` types. Unknown fields are
...
- **Implementation lands via follow-up issues; protocol.md + sage-protocol move
  ...
  concrete message types must land in `sage-protocol` **and** `docs/protocol.md` in
