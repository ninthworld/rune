...
as interactivity on that entity. The action bar shows subject-less actions plus a contextual
echo of the selected entity’s actions. Interaction is select then confirm on every input
method.
...
The server must maintain stable subject ids within each game view.
