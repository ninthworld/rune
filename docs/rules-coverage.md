...
| CR 704 | State-based actions are checked and applied to a fixed point after every action. | partial — the SBA loop exists; CR 704.5a and CR 704.5g are modeled (see below). | `sba.rs :: run_state_based_actions` | `sba.rs :: state_based_actions_reach_a_fixed_point` |
