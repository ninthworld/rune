...
mod tests {
    ...
    // ----- Double strike vs a multi-blocked, player-ordered attacker (issue #399) -----
    // Guards CR 510.1c–d (lethal-before-next along the chosen order) across the two
    // CR 702.4b damage steps, where the survivor set can change between them. The
    // existing issue_373_* tests only cover a SINGLE blocker; these exercise the #346
    // ordering machinery driven twice by a double striker.
    ...
    // ----- Commander damage: 21 combat damage from one commander (issue #371) -----
    //
    // CR 903.10a: a player dealt 21+ combat damage over the game by a single
    // commander loses. The tally is keyed to the commander *designation* (its
    // owning player), so it survives the commander's zone changes and recasts —
    // a fresh `PermanentId` on every battlefield entry never resets it. These tests
    // drive the real combat-damage seam (`deal_combat_damage`) and the SBA loop.
