...
//! room through [`sage_server::serve_connection`], receives its personalized
...
async fn serve(handle: sage_server::RoomHandle, ws: WebSocketStream<tokio::io::DuplexStream>) {
    sage_server::serve_connection(0, handle, ws, std::future::pending::<()>()).await;
}
