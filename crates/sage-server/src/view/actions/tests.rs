//! Projecting the engine's offered actions, checked against the offers a client
//! reads back.
...
/// **The view carries the cost the game will charge, and the one the card prints**
/// (issue #735, CR 601.2f).
///
/// The client draws what a spell costs and computes nothing, so the modified number
/// has to arrive as a number. Both halves ride together because the presentation is
/// a comparison — a discount is only legible against the printed cost beside it —
/// and the printed one is what the card keeps.
#[test]
fn issue_735_a_cast_states_its_printed_and_its_modified_cost() {
    let db = CardDatabase::bundled().unwrap();
    let mut state = GameState::new_two_player();
    state.step = Step::PrecombatMain;
    state.turn = 3;
    ...
    let wolves = state.new_instance(fixture("thornhide_wolves"));
    state.players[0].hand = vec![wolves];

    // Unmodified, the two halves agree and the cast is not affordable off three mana.
    let bare = personalized_view(&state, &db, PlayerId(0));
    assert!(
        !bare.valid_actions.iter().any(|a| a.kind == "cast_spell"),
        "{{4}}{{G}} is not castable off three mana"
    );

    put_permanent(
        &mut state,
        fixture("goreclaw_terror_of_qal_sisma"),
        PlayerId(0),
        false,
        false,
    );
    ...
    let cost = cast.cost.as_ref().expect("a cast states its cost");
    assert_eq!(cost.printed, "{4}{G}", "the card still says what it says");
    assert_eq!(cost.modified, "{2}{G}", "and the bar carries what it costs");

    // Every other action states no cost at all, so a client has nothing to draw and
    // nothing to infer.
    assert!(view
        .valid_actions
        .iter()
        .filter(|a| a.kind != "cast_spell")
        .all(|a| a.cost.is_none()));
}
