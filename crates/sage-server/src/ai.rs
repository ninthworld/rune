...
//! offered, exactly like the terminal agent (`sage-cli`).
//!
...
//! the available kinds into the [`CatalogView`](sage_protocol::CatalogView) so a client
...
use sage_protocol::{ChooseAction, ClientMessage, GameView, Prompt, TargetChoice, ValidAction};
...
/// `sage-server`'s view projection). The random policy prefers it as a safe fallback.
...
impl AiKind {
    ...
    /// [`AddAi::kind`](sage_protocol::AddAi::kind) carries and a
    /// [`SeatView::ai`](sage_protocol::SeatView::ai) reports.
    #[must_use]
    ...
    /// rejects an [`AddAi`](sage_protocol::AddAi) whose `kind` does not resolve here.
    #[must_use]
...
/// [`CatalogView`](sage_protocol::CatalogView) advertises (issue #415), so a client can
/// present the AI choices from server metadata rather than a hardcoded list.
#[must_use]
pub fn ai_options() -> Vec<sage_protocol::AiOption> {
    AiKind::all()
        .iter()
        .map(|kind| sage_protocol::AiOption {
...
mod tests {
    ...
    use super::*;
    use sage_protocol::{Phase, PromptOption};
