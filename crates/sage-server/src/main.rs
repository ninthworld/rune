//! SAGE server — layers 1 (lobby) and 2 (rooms) per docs/brief.md.
//! Owns networking, sessions, and timers. Never owns rules — that is sage-engine.
//!
//! This binary wires the [`sage_server`] transport to the process: it reads the
//! listen address from the environment/CLI, initialises logging, binds the
//! WebSocket listener, and serves until Ctrl-C.

use std::process::ExitCode;

use sage_server::{Config, Lobby, Server};
...
async fn main() -> ExitCode {
    ...
    info!(addr = %server.local_addr(), "sage-server listening");
