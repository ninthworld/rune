//! SAGE server transport — layers 1 (lobby) and 2 (rooms).
//!
//! This crate owns networking, sessions, and timers; it never owns rules — that
//! is [`sage_engine`]. Layer 1 (this module) runs a Tokio runtime, accepts
...
//! its own personalized [`sage_protocol::GameView`]. Redacting hidden zones and
...
///
/// Overridable via the `--addr <host:port>` CLI flag or the `SAGE_SERVER_ADDR`
...
/// [`Config::rng_seed`]).
pub const RNG_SEED_ENV_VAR: &str = "SAGE_RNG_SEED";

/// Environment variable read for a fixed starting life total (see
/// [`Config::starting_life`]).
pub const STARTING_LIFE_ENV_VAR: &str = "SAGE_STARTING_LIFE";
