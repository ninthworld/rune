...
/// [`rune_protocol::PlayerId`] — the seat key `controller`, `seat_order`, and
...
fn source_permanent(state: &GameState, source: PermanentId) -> Option<&rune_engine::Permanent> {
...
mod tests {
    ...
    fn issue_550_a_reconnect_mid_resolution_rebuilds_the_stack_and_its_paths() {
        ...
        let rebuilt: rune_protocol::GameView = serde_json::from_str(&wire).unwrap();
        assert_eq!(
