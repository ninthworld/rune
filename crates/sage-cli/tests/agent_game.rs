...
//!
//! Two [`RuleBasedAgent`](sage_cli::RuleBasedAgent) policies play a complete, legal
//! game against a real layer-2 [`sage_server::Room`] built from starter decks and a
...
//! with [`sage_cli::choose_action`] and filling every slot with
//! [`sage_cli::fill_answers`] — exactly what the socket loop
//! ([`run_agent_session`](sage_cli::run_agent_session)) sends on the wire, minus the
...
use sage_cli::{choose_action, fill_answers};
use sage_engine::{
...
use sage_server::{AutoPassPolicy, Room, RoomInput};
...
// ----- Structured game-log coherence over a full game (issue #259) -----
...
async fn play_seeded_game_collecting_log(seed: u64) -> Vec<(u64, GameLogEvent)> {
    ...
    let state = GameState::new(&setup, &db).expect("valid setup");
    // Automation off so the agents drive every pass themselves — the log records the
    // full manual sequence of step transitions and their consequences.
...
/// Read the bundled starter decks from the **single source of truth** both sides use:
/// the repository-root `data/starter-decks.json`. Reading that exact file here — rather
