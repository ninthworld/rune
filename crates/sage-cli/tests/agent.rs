...
//! ([`sage_server::Room`] + [`sage_server::serve_connection`]) over an in-memory
...
use sage_cli::{run_agent_session, Agent, AgentError, RuleBasedAgent};
use sage_engine::{CardDatabase, CardId, FunctionalId, GameSetup, GameState};
use sage_protocol::GameView;
use sage_server::{serve_connection, Room, RoomHandle, RoomInput, Seat};
...
/// by authored `functional_id` (ADR 0008 §3) — a `CardId` is interned from the catalog's
