...
//!
//! Four [`RuleBasedAgent`](sage_cli::RuleBasedAgent) policies play a complete, legal
//! free-for-all against a real layer-2 [`sage_server::Room`] seated for four players,
...
//! (never any engine access), deciding with [`sage_cli::choose_action`] and filling
//! every slot — including the multiplayer `defend_<id>` defender choice — with
//! [`sage_cli::fill_answers`], exactly what the socket loop sends on the wire.
//!
...
use sage_engine::{CardDatabase, CardId, FunctionalId, GameSetup, GameState, PlayerSetup};
