//! Non-interactive **agent mode** for the SAGE CLI (dev sequence step 4,
...
/// (`sage-server`'s `view.rs`). The safe default prefers this action.
...
pub const AGENT_TIMEOUT_ENV_VAR: &str = "SAGE_AGENT_TIMEOUT";
...
/// The rule-based decision policy (issue #159): `choose_action` and `fill_answers`
/// over a [`GameView`], plus the [`RuleBasedAgent`] that drives them. Split out of the
/// session runtime below (docs/coding-standards.md, File size); re-exported here so the
/// crate's public paths are unchanged.
mod policy;
pub use policy::{choose_action, fill_answers, RuleBasedAgent};
