//! Compile-time catalog assembly (ADR 0008 §4).
//!
...
//! engine (`crates/sage-engine/AGENTS.md`), and this script is not part of it: it runs
...
//! mechanism ADR 0002 already sanctioned for embedding card data. What ships still does
...
struct Definition {
    /// Its authored identity — also its file name (ADR 0008 §4).
...
/// sorted by `FunctionalId` byte value, which is what assigns `CardId(0..n)` (ADR 0008 §3).
///
...
/// Every printing must reference a definition the catalog actually holds (ADR 0008 §5).
///
...
fn render_manifest(definitions: &[Definition], sets: &[Set]) -> Result<String, Box<dyn Error>> {
    ...
    out.push_str(
        "// @generated by build.rs from data/catalog/ and data/sets/ — do not edit.\n\
         ...
         // value, interned to CardId(0..n) (ADR 0008 §3).\n\n",
    );
