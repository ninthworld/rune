...
//! exclusion list. Run it with `make compat` (or `cargo run -p sage-engine --bin
...
//!
//! The report text itself is produced by [`sage_engine::compat::bundled_report`], a
...
fn main() -> Result<(), Box<dyn Error>> {
    let report = sage_engine::compat::bundled_report()?;
