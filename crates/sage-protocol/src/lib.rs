//! SAGE protocol — the entire client/server contract.
//!
...
//! root so every type is reachable as `sage_protocol::Foo` regardless of where it
//! is defined:
...
pub use action::{ManaOption, NumberValue, Prompt, PromptOption, TargetRequirement, ValidAction};
pub use card::{
