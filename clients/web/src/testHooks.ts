 ...
 *
 * The legacy Pixi canvas and the ADR 0030 DOM plane are pure visual surfaces
 * whose output is fully determined by the view the client already holds. Rather
 * than decode pixels or infer DOM layout, an e2e test reads the corresponding
 * derived scene here and asserts on
 ...
 * statically `undefined`, so the publishers short-circuit and never
 * touches `window` — the hook cannot ship live to players.
 */
import type { GameView } from './protocol';
import type { StagedPlane } from './table/plane';
...
export interface RuneTestHook {
  /** The latest legacy {@link TableScene}, or `null` when that path is not mounted. */
  scene: TableScene | null;
  /** The latest ADR 0030 {@link StagedPlane}, or `null` when that path is not mounted. */
  plane: StagedPlane | null;
  /**
...
function hook(): RuneTestHook | null {
  ...
  const existing = window.__RUNE_TEST__ ?? { scene: null, plane: null, view: null };
...
/**
 * Publish the current ADR 0030 scene plane on `window.__RUNE_TEST__` when the
 * test hook is enabled. A no-op in production builds and without `window`.
 */
export function publishPlane(plane: StagedPlane | null): void {
  const surface = hook();
  if (surface) surface.plane = plane;
}
