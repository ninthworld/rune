...
export function Game({
  ...
  // How many rows a field divides into, decided for the *table* rather than per seat, so a
  // permanent is the same size at both ends of it and nobody's board is two thirds the size of the
  // other's. Both fields have the same box, so either of them states the room.
  //
  // What is handed over is how many *groups* each half drew and nothing else: the row count is the
  // board's (§5), one row per group the server's `card_types` produced, and it does not fall to buy
  // card size. How many permanents are in a group is the cards' problem — they get smaller — so it
  // is not passed, because a number a decision must not read is an invitation to read it.
