 ...
 *
 * The decision this asks a player to make is a real one, so it is written out rather than
 * reduced to a toggle: turning on a third-party source means *this browser* will request card
 ...
 *
 * Two exports: the controls, which the game's settings panel embeds as one of its sections, and
 * the standalone dialog the lobby opens — the lobby has no settings panel to embed them in, and
 * before a game starts is the one moment a player has time to read what they are turning on.
 */
      ...
      {preference.source === 'scryfall' && (
        <fieldset className="art-settings__group">
          ...
          <label>
            ...
            />{' '}
            <strong>Illustration only</strong> — inside SAGE&rsquo;s frame, which keeps drawing the
            name, cost, type, and stat.
          </label>
          ...
          <label>
            ...
            />{' '}
            <strong>Whole card</strong> — the card image is the face. Everything the server computes
            still draws on top of it, so a buffed creature never reads as its printed numbers.
          </label>
        </fieldset>
      )}
