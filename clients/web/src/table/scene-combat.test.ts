...
describe("buildTableScene combat state (issue #332)", () => {
  ...
  const byId = (scene: TableScene) =>
    new Map(allCards(scene).map((c) => [c.entityId, c]));
  ...
  it("never folds a combat participant into an ×N stack", () => {
    ...
    const scene = build(
      parseGameView(
        JSON.stringify({
          ...
          my_hand: [],
          opponents: [
            { player_id: "p2", hand_size: 0, life: 20, library_size: 40 },
          ],
          battlefield: [
            {
              id: "a",
    ...
    expect(
      local.cards.every((c) => c.stackCount === 1 && c.data.attacking),
    ).toBe(true);
  });
  ...
  it("has no combat links or attacking flags outside combat", () => {
    ...
    expect(allCards(scene).every((c) => !(c.data.attacking ?? false))).toBe(
      true,
    );
  });
});
