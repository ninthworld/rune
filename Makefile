.PHONY: verify check client-check client-install engine-test engine-lint engine-fmt compat deny setup
...
# fast inner loop.
#
# The browser e2e suite is deliberately not here: ADR 0011 keeps it a separate
# job so engine-only work never waits on a browser, and `make check` stays
# green-able without one.
verify: check client-check deny ## Full pre-merge verification: Engine + Client + cargo-deny (mirrors every required GitHub check)
